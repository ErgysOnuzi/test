  ...
  const loadMoreImages = async () => {
    ...
    try {
      const response = await fetch(
        `/api/gallery?offset=${nextOffset}&limit=36`
      );
      if (response.ok) {
        const data = await response.json();
      ...
      <div className='text-center py-20'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-8 mt-16'>
          <div className='bg-card/50 backdrop-blur-sm rounded-3xl p-8 border border-border hover:shadow-xl transition-all duration-300'>
            ...
            <p className='text-foreground/75'>{t('beautiful_photos')}</p>
          </div>
          ...
          <div className='bg-card/50 backdrop-blur-sm rounded-3xl p-8 border border-border hover:shadow-xl transition-all duration-300'>
            ...
            <p className='text-foreground/75'>{t('special_events_desc')}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className='relative'>
      ...
      {showLoadingOverlay && (
        <GalleryLoadingAnimation
          ...
          loadedImages={
            timeoutReached ? images.length : Math.min(loadedImagesCount, 6)
          }
        />
      )}
      ...
      {/* Gallery Content */}
      <div
        className={`transition-all duration-1000 ${showGallery ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      >
