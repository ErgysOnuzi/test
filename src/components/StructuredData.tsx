    ...
    "makesOffer": [
      {
        ...
        "itemOffered": {
          ...
          "description": locale === 'de' ? "Täglich frisch zubereitete Pasta" : "Daily fresh prepared pasta"
        }
      },
      {
        "@type": "Offer", 
        "itemOffered": {
          ...
          "description": locale === 'de' ? "Authentische neapolitanische Pizza aus dem Holzofen" : "Authentic Neapolitan pizza from wood oven"
        }
      }
    ]
  };
...
export function MenuSchema({ menuItems, locale }: { menuItems: any[], locale: string }) {
  const menuData = {
    "@context": "https://schema.org",
    "@type": "Menu",
    "hasMenuSection": [
      {
        ...
        "hasMenuItem": menuItems.filter(item => !item.category?.toLowerCase().includes('drink')).map(item => ({
          "@type": "MenuItem",
      ...
      },
      {
        ...
        "hasMenuItem": menuItems.filter(item => item.category?.toLowerCase().includes('drink') || item.category?.toLowerCase().includes('bier') || item.category?.toLowerCase().includes('wein')).map(item => ({
          "@type": "MenuItem",
          "name": locale === 'de' ? item.titleDe || item.title : item.titleEn || item.title,
          "description": locale === 'de' ? item.descriptionDe || item.description : item.descriptionEn || item.description,
          "offers": {
...
export function generateSEOMetadata(
  ...
  const data = pageData[page]?.[locale as 'de' | 'en'] || pageData[page]?.['de'] || {
    title: 'Ristorante La Cantina Bleibtreu - Authentic Italian Restaurant Berlin',
    description: 'Experience authentic Italian cuisine at Ristorante La Cantina Bleibtreu in Berlin. Fresh pasta, traditional recipes, and warm hospitality await you.'
  };
